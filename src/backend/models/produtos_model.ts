...
class ProdutosModel {
  ...
  getCell (idCollumn: string, targetCollumn: string, id_produto: number, callback: queryCallback) {
    const query = `SELECT ${targetCollumn} FROM tb_products WHERE ${idCollumn} = ${id_produto};`;
    db.query(query, callback);
  } 
  alterLinha(id_produto: number, targetCollumn: string, value: string | number, id_Collumn: string,callback: queryCallback): any {

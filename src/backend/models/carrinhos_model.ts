...
class CarrinhoModel {
    ...
    create_Carrinho_Produto(id_cliente: number, id_produto: number, Qt_Product_Carrinho: number, callback:queryCallback) {
    ...
    remover_Produto_carrinho(id_carrinho_produto: number, callback:queryCallback) {
        const query = `DELETE FROM tb_carrinho_produtos WHERE id_carrinho_produto = ${id_carrinho_produto}`;
        db.query(query, callback);
    }
    alt_Qt_Carrinho_Produto(id_carrinho_produto: number, Qt_Product_Carrinho:number, callback:queryCallback) {
